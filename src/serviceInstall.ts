...
export function installService(): ServiceResult {
  const p = platform();
  if (p === "darwin") return installLaunchd();
  if (p === "linux") return installSystemdUser();
  if (p === "win32") return installWindowsTask();
...
export function uninstallService(): ServiceResult {
  ...
  if (p === "linux") return uninstallSystemdUser();
...
function installLaunchd(): ServiceResult {
...
function installSystemdUser(): ServiceResult {
  ...
  if (probe.error) {
    return {
      ok: false,
      message:
        "systemctl not found · run `claudex start` manually, or wrap in your own supervisor",
    };
  }
  mkdirSync(LOG_DIR, { recursive: true });
...
function uninstallSystemdUser(): ServiceResult {
...
function installWindowsTask(): ServiceResult {
  mkdirSync(LOG_DIR, { recursive: true });
...
function uninstallWindowsTask(): ServiceResult {
  spawnSync("schtasks", ["/end", "/tn", WIN_TASK], {
    stdio: "ignore",
    shell: true,
  });
