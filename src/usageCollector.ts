...
import type { TokenBreakdown, ModelUsage, SessionInfo, TopProject } from "./protocol.js";
import { priceFor } from "./pricing.js";

/**
 * Claude Code session lines in ~/.claude/projects/<proj>/<session>.jsonl
 * Each message has a `message.usage` block when it's an assistant turn.
 */
...
export function computeCost(model: string, u: TokenBreakdown): number {
  const p = priceFor(model);
  return (
    (u.input * p.input) / 1_000_000 +
...
export interface AggregatedUsage {
  ...
  /** Month-to-date API-equivalent cost in USD, from 1st of the current local calendar month. */
  monthlyCostUSD: number;
  /** Day of month the stats began — used by the UI to pro-rate projections. */
 ...
 */
export async function aggregateFromJSONL(
  hours = 24,
  basePath?: string,
  /**
  ...
  const perModel = new Map<string, { tokens: TokenBreakdown; requests: number }>();
  const perProject = new Map<string, { costUSD: number; requests: number; tokens: number }>();
  const total = emptyBreakdown();
  let totalRequests = 0;

  // 7 calendar days of cost, index 0 = today (local), index 6 = 6 days ago.
  const weeklyCostUSD = new Array<number>(7).fill(0);

  // Hourly usage today (local time), index 0 = 00h
  const hourlyUsageUSD = new Array<number>(24).fill(0);
  ...
  // the end.
  const windowCandidates: Array<{ ts: number; billable: number; total: number }> = [];
  ...
  for (const proj of projects) {
    ...
    for (const f of files) {
      ...
      if (mtime < fileScanCutoff) continue; // skip ancient files entirely
      ...
      for (const line of content.split("\n")) {
        ...
        if (kind === "user" && ts >= Date.now() - sessionScanMs) {
          ...
          if (Array.isArray(content)) {
            ...
            if (allToolResult) isRealPrompt = false;
          }
          if (isRealPrompt) userMessageTimestamps.push(ts);
        }
        ...
        const tokenSum =
          breakdown.input + breakdown.output + breakdown.cacheCreation + breakdown.cacheRead;
  ...
  for (const c of windowCandidates) {
    if (c.ts >= windowStart) session.windowUsed += c.billable;
  }

  // Count real user messages in the current 5h window.
  for (const ts of userMessageTimestamps) {
    if (ts >= windowStart) session.windowMessages += 1;
  }

  const models: ModelUsage[] = [...perModel.entries()]
    .map(([model, v]) => ({
      model,
      tokens: v.tokens,
      requests: v.requests,
  ...
  const topProjects: TopProject[] = [...perProject.entries()]
    .map(([path, v]) => ({
...
function daysAgoLocal(ts: number, startOfTodayMs: number): number {
  if (ts >= startOfTodayMs) return 0;
  const d = new Date(ts);
  d.setHours(0, 0, 0, 0);
...
export function friendlyProjectName(path: string): string {
...
function emptyAggregated(): AggregatedUsage {
  const now = Date.now();
...
export function parseUsageOutput(text: string): { windowLimit: number | null; windowPercent: number | null } {
