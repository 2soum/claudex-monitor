...
import { aggregateFromJSONL, runUsageSlashCommand, parseUsageOutput, } from "./usageCollector.js";
...
// off.
const WINDOW_LIMIT_OVERRIDE = process.env.CLAUDE_WINDOW_LIMIT ? Number(process.env.CLAUDE_WINDOW_LIMIT) : null;
 ...
 * the cache.
 */
async function buildSnapshot(windowStartOverride) {
...
async function buildSnapshotFresh(windowStartOverride) {
    {
        ...
        let authoritativePercent = null;
        if (usageText) {
            const parsed = parseUsageOutput(usageText);
            if (parsed.windowLimit)
                session.windowLimit = parsed.windowLimit;
        ...
        const tokensPerMinute = recent.length > 0 ? (recent.reduce((s, v) => s + v, 0) / recent.length) * 12 : 0;
        const remaining = session.windowLimit ? session.windowLimit - session.windowUsed : 0;
        const projectionMinutesToLimit = tokensPerMinute > 0 && remaining > 0 ? remaining / tokensPerMinute : null;
        const snap = {
            timestamp: Date.now(),
...
function sampleTPM() {
    ...
    if (!snap)
        return; // no snapshot yet — wait for first build
    const billable = snap.totalTokens.input + snap.totalTokens.output + snap.totalTokens.cacheCreation;
    if (tpmInitialized) {
        const delta = Math.max(0, billable - lastBillableTotal);
        tpmSeries.push(delta);
        if (tpmSeries.length > TPM_WINDOW)
            tpmSeries.shift();
    }
    else {
        tpmInitialized = true;
...
async function refreshLimit() {
    try {
        ...
        console.warn("[limit] inference failed:", e.message);
    }
}
refreshLimit();
